...
*/
float render_pixel(float *imgIn, int yOut, int xOut, unsigned int mIn,
                   unsigned int nIn, unsigned int mOut, unsigned int nOut,
                   unsigned int offset, unsigned int nMonteCarlo,
                   float grainRadius, float sigmaR, float sigmaFilter,
                   float xA, float yA, float xB, float yB, float *lambdaList,
                   float *expLambdaList, float *xGaussianList, float *yGaussianList)
{
    float normalQuantile = 3.0902;	//standard normal quantile for alpha=0.999
    ...
    float currRadius,currGrainRadiusSq = grainRadiusSq;
    ...
    float xIn = xA + (xOut+(float)0.5 ) * ((xB-xA)/(nOut));
    float yIn = yA + (yOut+(float)0.5 ) * ((yB-yA)/(mOut));
    ...
    for (unsigned int i=0; i<nMonteCarlo; i++)
    {
        ...
        for(int ncx = minX; ncx <= maxX; ncx++) /* x-cell number */
        {
            ...
            for(int ncy = minY; ncy <= maxY; ncy++) /* y-cell number */
            {
                ...
                float u = imgIn[(unsigned int)(fmin(fmax(floor(cellCorner.y),0.0),
                                                    (float)(mIn-1)))*nIn+
                ...
                for(unsigned int k=0; k<Ncell; k++)
                {
                    ...
                    {
                        std::cout << "Error, the standard deviation of the grain should be positive.";
                        std::cout << std::endl;
                    }

                    // test distance
                    if(sqDistance(xCentreGrain,yCentreGrain, xGaussian,yGaussian)
                            < currGrainRadiusSq)
                    {
...
#pragma omp parallel for schedule(static) private(i,j,pt,pix) shared(imgOut)
    for (i=0; i<(unsigned int)imgOut->get_nrows(); i++)
    {
        ...
        for (j=0; j<(unsigned int)imgOut->get_ncols(); j++)
        {
            ...
            pix = render_pixel(pt, i, j, (unsigned int)imgIn->get_nrows(),
                               (unsigned int)imgIn->get_ncols(),
...
matrix<float>* boolean_model(float lambda, float r, float stdGrain)
{
    ...
    matrix<float> * grainModelOut = new matrix<float>(nDots,3);
                                               ...
                                               filmGrainOptionsStruct<float> filmGrainOptions)
{
    ...
    float sigmaFilter = filmGrainOptions.sigmaFilter;
    float sX = ((float)filmGrainOptions.nOut)
    ...
    for (int i=0; i<NmonteCarlo; i++)
    {
        X->set_value(i,0,sigmaFilter*(float)myrand_gaussian_0_1(&pMonteCarlo));
        Y->set_value(i,0,sigmaFilter*(float)myrand_gaussian_0_1(&pMonteCarlo));
    }


    //optionally, save the image of values of lambda
    matrix<float> *imgLambda = new matrix<float>(imgIn->get_nrows(),
                                                 imgIn->get_ncols());
    //create output image
    matrix<float> *imgOut = new matrix<float>( filmGrainOptions.mOut,
                                               filmGrainOptions.nOut );
    imgOut->set_to_zero();
    //create list of temporary images, and set them to 0.
    //We use these to store the result of each Monte Carlo evaluation
                                 ...
                                 )
                            {
                                        ...
                                        ( ( (float)x/sX) -xGrainTemp)*(((float)x/sX)-xGrainTemp)<=rGrainTempSq)==true)
                                {
                                    (imgTempptr[k])->set_value( (int)((float)y - floor( (filmGrainOptions.yA)*sY)),
                                                                (int)fmin(fmax( (float)x  - floor( (filmGrainOptions.xA) *sX) , 0),
                                                                          imgOut->get_ncols()-1), (bool) 1 );
                                }
                            }
                        }
                    }
